...
Camera::Camera(const Vec4 _origin, const Vec4 _forward, const Vec4 _up,
               int width, int height)
    : origin(_origin), forward(_forward), up(_up) {
...
PPMImage Camera::render(int width, int height, int rpp,
    ...
    const Vec4 deltaX =
        cameraToWorld(Vec4((2.0f / width) * ratio, 0, 0.0f, 0.0f));
